...
async def run_once(trigger: str = "auto_loop") -> dict:
    ...
    锁被占用（手动/开机钩子运行中）时跳过本轮（防并发，不写盘），返回占位。
    """
    lock = _get_lock()
    if lock.locked():
    ...
    async with lock:
        ...
        return await asyncio.to_thread(evolve.run_evolve, None, trigger)
...
async def run_protected(fn) -> dict:
    ...
    锁占用时返回跳过占位，避免并发。所有 run_evolve 入口必须经此函数进入。
    """
    lock = _get_lock()
    if lock.locked():
        return {"ledger_new": [], "no_op": True, "skipped": True,
...
def start() -> None:
    ...
    _state["enabled"] = True
    _state["task"] = asyncio.ensure_future(_loop())
